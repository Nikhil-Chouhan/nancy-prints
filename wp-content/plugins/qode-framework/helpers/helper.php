...
if ( ! function_exists( 'qode_framework_get_option_value' ) ) {
	...
	function qode_framework_get_option_value( $scope, $type, $name, $default_value = '', $post_id = null ) {
		...
		} elseif ( $type == 'customizer' ) {
			$value = get_option( $name );
		}
		
		$value = isset( $value ) && ( ! empty( $value ) || $value == '0' ) ? $value : $default_value;
...
if ( ! function_exists( 'qode_framework_wp_kses_html' ) ) {
	...
	function qode_framework_wp_kses_html( $type, $content ) {
		switch ( $type ) {
			case 'img':
				$atts = array(
					'img' => apply_filters( 'qode_framework_filter_wp_kses_img_atts', array(
						'itemprop' => true,
						'id'       => true,
						'class'    => true,
						'width'    => true,
				...
				break;
			case 'svg':
				$atts = apply_filters( 'qode_framework_filter_wp_kses_svg_atts', array(
					'svg'   => array(
						'xmlns'             => true,
						'version'           => true,
						'id'                => true,
				...
				break;
			case 'content':
				$atts = apply_filters( 'qode_framework_filter_wp_kses_content_atts', array(
					'div' => array(
						'id'    => true,
						'class' => true,
						'style' => true,
...
if ( ! function_exists( 'qode_framework_get_hours_range' ) ) {
	 ...
	 *
	 * @param array $params  - config param
	 *
	 * @return array
	 */
	function qode_framework_get_hours_range( $params = array() ) {
		// Default values
		$start  = 0;
		$end    = 86400;
		$step   = 1800;
...
if ( ! function_exists( 'qode_framework_get_people_range' ) ) {
	...
	function qode_framework_get_people_range() {
		...
		for ( $i = 1; $i <= 20; $i ++ ) {
			$options[ $i ] = sprintf( '%s %s', $i, $i === 1 ? esc_html__( 'Person', 'qode-framework' ) : esc_html__( 'People', 'qode-framework' ) );
		}
