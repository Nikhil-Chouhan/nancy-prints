...
class QodeFrameworkElementorTranslator {
				...
				if ( ! empty( $params[ $repeater_param ] ) ) {
					...
					foreach ( $option['items'] as $item_key => $item ) {
						...
						if ( $item['field_type'] == 'image' ) {
							...
							if ( ! isset( $item['multiple'] ) || ( isset( $item['multiple'] ) && $item['multiple'] != 'yes' ) ) {
	...
	private function get_shortcode_snippet( $shortcode_object, $params ) {
		...
		return sprintf( '<textarea rows="3" readonly>[%s %s]</textarea>',
	...
	function create_render( $shortcode_object, $params ) {
		...
		if ( isset( $params['shortcode_snippet'] ) && $params['shortcode_snippet'] === 'yes' ) {
